...
#ifndef WINDOW_TITLE 
#define WINDOW_TITLE "Tmpl8-2024" 
#endif 
...
int main()
{
...
#ifdef FULLSCREEN
	window = glfwCreateWindow(SCRWIDTH, SCRHEIGHT, WINDOW_TITLE, glfwGetPrimaryMonitor(), 0);
#else
	window = glfwCreateWindow(SCRWIDTH, SCRHEIGHT, WINDOW_TITLE, 0, 0);
#endif
	...
	InitRenderTarget(SCRWIDTH / 4 , SCRHEIGHT / 4);
	Surface* screen = new Surface(SCRWIDTH / 4, SCRHEIGHT / 4);
	app = new Game();
	app->screen = screen;
	app->Init();
...
//CREDITS TO: https://github.com/wessles/GLSL-CRT/blob/master/shader.frag
	...
	//	"#define CRT_CURVE_AMNTx 1.0 // curve amount on x\n"
	//	"#define CRT_CURVE_AMNTy 1.0 // curve amount on y\n"
	//	"#define CRT_CASE_BORDR 1.0125\n"
	//	"#define SCAN_LINE_MULT 1100.0\n"
	//	"in vec2 u;\n"
	//	"out vec4 fragColor; \n"
	//	"uniform sampler2D c; \n"
	//	"void main() {\n"
	//	"vec2 tc = vec2(u.x, u.y); \n"
	...
	//	"tc.y += 0.5; \n"
	//	"vec4 cta = texture2D(c, vec2(tc.x, tc.y)); \n"
	...
	//Shader* shader = new Shader(
		//"#version 330\nin vec4 p;\nout vec2 u;void main(){u=vec2((p.x+1)/2,1-(p.y+1)/2);gl_Position=vec4(p.x,p.y,1,1);}", fs, true);
#else
	// fxaa shader
	Shader* shader = new Shader(
		"#version 330\nin vec4 p;\nout vec2 uv;void main(){uv=vec2((p.x+1)/2,1-(p.y+1)/2);gl_Position=vec4(p.x,p.y,1,1);}",
